...
/**
 * @var $dataPerStation array<string, StationData> Keys are the station name
 */
$dataPerStation = [];
...
while (($line = fgets($fileHandle)) !== false) {
    ...
    if (!isset($dataPerStation[$name])) {
        $dataPerStation[$name] = new StationData($value, $value, $value);

        continue;
    }

    $data = $dataPerStation[$name];
...
foreach ($dataPerStation as $name => $data) {
    $results[$name] = [
        'min' => $data->min,
