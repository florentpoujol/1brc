...
// from https://github.com/realFlowControl/1brc/blob/main/createMeasurements.php

class WeatherStation {
    private $id;
    private $meanTemperature;

    public function __construct($id, $meanTemperature) {
        $this->id = $id;
        $this->meanTemperature = $meanTemperature;
    }

    public function measurement() {
        $m = $this->gauss_ms($this->meanTemperature, 10);
        return round($m * 10) / 10.0;
    }

    // PHP lacks a native Gaussian/normal distribution function, so we implement the Box-Muller transform
    private function gauss_ms($mean, $stdDev) {
...
function createMeasurements(int $numRecords, string $fileNameSuffix): void
{
    ...
    $fileHandle = fopen($measurementFile, 'w');

    if (!$fileHandle) {
        throw new Exception("Cannot open file: " . $measurementFile);
    }

    for ($i = 0; $i < $numRecords; $i++) {
        if ($i > 0 && $i % 50000 === 0) {
            $timeElapsed = microtime(true) - $start;
            echo "Wrote $i measurements in $timeElapsed ms\n";
        }
        $station = $stations[array_rand($stations)];
        fwrite($fileHandle, $station->getId() . ";" . $station->measurement() . "\n");
    }

    fclose($fileHandle);
    $totalTime = microtime(true) - $start;
    echo "Created file with $numRecords measurements in $totalTime ms\n";
}
